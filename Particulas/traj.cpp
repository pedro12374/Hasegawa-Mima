...
double dydt2(double t,double x,double y,double *w, double *An, double *kx, double *ky){
  ...
  return An[1]*kx[1]*cos(kx[1]*x)*cos(ky[1]*y) + An[0]*kx[0]*cos(kx[0]*x)*cos(ky[0]*(y-(w[0]/ky[0] - w[1]/ky[1])*t))+ An[2]*kx[2]*cos(kx[2]*x)*cos(ky[2]*(y-(w[2]/ky[2] - w[1]/ky[1])*t));
}

double dxdt2(double t,double x,double y,double *w, double *An, double *kx, double *ky){

  return An[1]*ky[1]*sin(kx[1]*x)*sin(ky[1]*y) + An[0]*ky[0]*sin(kx[0]*x)*sin(ky[0]*(y-(w[0]/ky[0] - w[1]/ky[1])*t)) + An[2]*ky[2]*sin(kx[2]*x)*sin(ky[2]*(y-(w[2]/ky[2] - w[1]/ky[1])*t));
}
...
int main(int argc, char const *argv[]) {
  ...
  string phi_f = argv[2];
  double x = atof(argv[3]); // x' inicial (já normalizado)
  double y = atof(argv[4]); // y' inicial (já normalizadtmax = atof(argv[4]);  // numero de interaçõe
  ...
  infile.open("phi/phi_"+phi_f+".dat");// file containing numbers in 3 columns 
     if(infile.fail()) // checks to see if file opended 
    { 
      cout << "error" << endl; 
      return 1; // no point continuing if the file didn't open...
    } 
double strobo  = (1.31e-3)*ky[0]*fabs(1/ky[1]-1/ky[0]);
double tfile;
  //  Loop de integracao
  int counter = 0;
  while (t <= tmax) {
    ...
    if(counter%1000==0){
    //if(  fmod( t, 2*M_PI/(strobo*1000)  )>=-0.001 && fmod(t,2*M_PI/(strobo*1000)      )<=0.001 ){
