...
void write_lorenz( const state_type &x , const double t )
{
  cout.precision(17);
  if(fmod(t,1)==0){
    cout<<t<<"\t"<< fixed  << x[0].real() << '\t'<<fixed  << x[1].real()  << '\t'<<fixed << x[2].real()<<'\t' <<fixed <<  x[0].imag() << '\t'<<fixed  << x[1].imag()  << '\t'<<fixed  << x[2].imag()<< endl;
  }
}
...
int main() {
    ...
    vector<double> psi2;
        ...
        size_t steps = integrate_adaptive( stepper , mima(-0.1,0.01,-0.1) , x , 0.0 , 500.0 , 1e-3,write_lorenz);
