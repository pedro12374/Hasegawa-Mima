...
struct mima
{
    ...
    {
        dHdt[0] = ky1*sin(kx1*H[0])*sin(ky1*H[1]);//+A2*sin(t)*sin(t)*ky2*sin(kx2*H[0])*sin(ky2*(H[1]-u*t)); 
        dHdt[1] = kx1*cos(kx1*H[0])*cos(ky1*H[1]);//+A2*sin(t)*sin(t)*kx2*cos(kx2*H[0])*cos(ky2*(H[1]-u*t)); 
    }
};
...
int main() {
	...
	for (double x_p=x_i; x_p<=x_f; x_p+=x_step) {
		for (double y_p=y_i; y_p<=y_f; y_p+=y_step) {
			x[0] =x_p; 
   			x[1] =y_p; 
			integrate_adaptive( stepper , mima() , x , 0.0 ,1000.0 , 1e-3, write_func);
